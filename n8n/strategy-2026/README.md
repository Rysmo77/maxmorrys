...
| **WF-SOCIAL-04** | `Build — URL publique` | Nouveau choix de gabarit : `story` → **`ask`**, `carrousel` → **`slide`**, le reste → **`poster`** (le style retenu par le board). Carte d'accent alignée sur le skill. **Le fond image n'est plus demandé** pour ces trois gabarits. |
...
2. **Déployer les nouveaux gabarits de créas** — sinon `ask` et `slide` n'existent pas côté rendu et
   `renderSocialCard` retombe sur `quote` :
   ```bash
   cd functions && npm run build && firebase deploy --only functions:renderSocialCard
   ```
...
- **`WF-SOCIAL-04` génère toujours une image en amont** (Gemini puis R2) alors que `poster`, `slide`
  et `ask` ne la consomment plus. Le patch ne change que le choix du gabarit, pas la topologie du
  workflow. Court-circuiter ces nœuds est une économie réelle, mais c'est une modification de
  structure — donc une décision board, pas un patch de prompt.
