...
Les 37 composants du système. **Lis ces contrats : ils portent les raisons, pas seulement
les types.** Plusieurs props encodent une décision que le nom seul ne dit pas — `level` sur
`GlassPanel` porte le budget de flou et la distinction voile / opaque, `safeBottom` sur
`TabBar` porte la zone de geste système.
