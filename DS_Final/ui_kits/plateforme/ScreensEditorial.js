  ...
  return (
    <Screen territory="informe" bar={<AppBar left={<BackButton onClick={()=>go&&go('blog')} />} right={<IconButton label="Partager"><Icon name="share" size={18} strokeWidth={2} /></IconButton>} />}>
      ...
      <GlassPanel level="flat" padding="6px 18px" className="rv" style={{'--i':9,marginTop:'10px'}}>
        <LessonRow state="plain" title="Puis-je payer une formation en plusieurs fois ?" trailing={<Icon name="forward" size={16} color="var(--text-faint)" strokeWidth={2.4} />} />
        <LessonRow state="plain" title="Que se passe-t-il si mon paiement échoue ?" trailing={<Icon name="forward" size={16} color="var(--text-faint)" strokeWidth={2.4} />} />
        <LessonRow state="plain" title="Combien de temps garde-t-on l'accès à une formation ?" trailing={<Icon name="forward" size={16} color="var(--text-faint)" strokeWidth={2.4} />} last />
      </GlassPanel>
...
(window.MMSIGN = window.MMSIGN || []).push('ScreensEditorial.js');
