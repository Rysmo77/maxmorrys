const { GlassPanel, Button, ChipRow, Field, LessonRow, QuotaMeter, Tag, Icon, IconButton, Segmented } = window.DS;
  ...
  return (
    <Screen territory="transforme"
      ...
      <GlassPanel level="hero" padding={20} className="rv" style={{'--i':4,marginTop:'18px'}}>
        ...
        {/* `ChipRow` en `layout="wrap"`, pas une réimplémentation : ces pilules l'avaient
            recopiée à 15 px de rembourrage au lieu de 16. Quatre suggestions plutôt qu'un
            champ vide — un champ vide fait hésiter. */}
        <div style={{marginTop:'14px'}}>
          <ChipRow layout="wrap" value={nom} onChange={(p)=>{ setNom(p); setTutorNom(p); }}
            options={['Répétiteur','Prof','Coach','Tonton']} />
        </div>
