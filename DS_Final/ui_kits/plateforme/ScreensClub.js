...
function Bilan({i=0}){
  /* `ink`, pas `night` : le Club est un territoire CLAIR. Un voile nuit y composerait avec le
     fond et remonterait à un gris moyen — 2,61:1 sous ce texte. Opaque, la carte ouvre sa
     portée .dk et les jetons de texte font le reste. */
  return (
    <GlassPanel level="ink" padding={18} className="rv" style={{'--i':i}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'12px'}}>
        <div>
          ...
          <p style={{fontFamily:'var(--f-display)',fontWeight:900,fontSize:'19px',letterSpacing:'-.032em',margin:'5px 0 0',color:'var(--text-body)'}}>Ce qu'il t'a apporté</p>
        </div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'12px'}}>
        ...
        <b className="mm-num" style={{fontSize:'12.5px',color:'var(--text-body)'}}>14/02/2027 · avis 15 j avant</b>
      </div>
    </GlassPanel>
  );
}
  ...
  return (
    <Screen territory="transforme" bar={<ClubBar go={go} titre="Membre" />}>
      ...
      <GlassPanel level="flat" padding="6px 18px" className="rv" style={{'--i':5,marginTop:'10px'}}>
        <LessonRow state="plain" icon={<Icon name="comment" size={14} />} title="J'ai refait ma fiche Google en suivant le module 2." meta="il y a 2 h · 12 j'aime" trailing={<Icon name="forward" size={16} color="var(--text-faint)" strokeWidth={2.4} />} />
        <LessonRow state="plain" icon={<Icon name="comment" size={14} />} title="Ma liste de 20 mots, si ça sert à quelqu'un." meta="hier · 8 j'aime" trailing={<Icon name="forward" size={16} color="var(--text-faint)" strokeWidth={2.4} />} />
