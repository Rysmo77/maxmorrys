...
async function getActiveRysmoSubscription(uid: string): Promise<string | null> {
...
async function getCatalog(): Promise<{ text: string; paths: Set<string> }> {
  ...
  try {
    ...
    if (!formationsSnap.empty) {
      const lines = formationsSnap.docs.map((d) => {
        const f = d.data();
        if (f.slug) paths.add(normPath(`/formations/${f.slug}`));
        const price = f.promoPrice ?? f.price;
        const priceLabel = price > 0 ? `${price} FCFA` : 'gratuit';
        return `- "${f.title}" — ${f.category ?? ''} · ${f.level ?? ''} · ${priceLabel} → /formations/${f.slug}`;
      });
    ...
    if (!blogSnap.empty) {
      const lines = blogSnap.docs.map((d) => {
        const a = d.data();
        if (a.slug) paths.add(normPath(`/blog/${a.slug}`));
        return `- "${a.title}" [${a.category ?? 'général'}] — ${truncate(a.excerpt, 120)} → /blog/${a.slug}`;
      });
    ...
    if (!podcastsSnap.empty) {
      const lines = podcastsSnap.docs.map((d) => {
        const p = d.data();
        if (p.slug) paths.add(normPath(`/podcasts/${p.slug}`));
        return `- "${p.title}" [${p.category ?? 'général'}] — ${truncate(p.description, 120)} → /podcasts/${p.slug}`;
      });
    ...
    if (!videosSnap.empty) {
      const lines = videosSnap.docs.map((d) => {
        const v = d.data();
        if (v.slug) paths.add(normPath(`/videos/${v.slug}`));
        return `- "${v.title}" [${v.category ?? 'général'}] — ${truncate(v.description, 120)} → /videos/${v.slug}`;
      });
...
function sanitizeInternalLinks(reply: string, validPaths: Set<string>): string {
...
async function getEnrolledText(uid: string): Promise<{ text: string; paths: Set<string> }> {
  const paths = new Set<string>();
  try {
    const db = admin.firestore();
    const enrSnap = await db.collection('enrollments').where('userId', '==', uid).get();
    if (enrSnap.empty) return { text: '', paths };
    ...
    const formationDocs = await Promise.all(
      enrollments.slice(0, 30).map((e) => db.doc(`formations/${e.formationId}`).get()),
    );
...
async function getMemoryConsent(uid: string): Promise<boolean> {
  try {
    const snap = await admin.firestore().doc(`users/${uid}`).get();
    // Activée par défaut : seul un opt-out explicite (false) la désactive.
    return snap.data()?.preferences?.aiMemoryConsent !== false;
  } catch {
    return false;
  }
}

async function getProfileText(uid: string): Promise<string> {
  try {
    const snap = await admin.firestore().doc(`rysmoProfiles/${uid}`).get();
    if (!snap.exists) return '';
    const p = snap.data()!;
    const parts: string[] = [];
    if (p.summary) parts.push(`Résumé : ${p.summary}`);
    if (Array.isArray(p.topics) && p.topics.length) parts.push(`Sujets d'intérêt : ${p.topics.join(', ')}`);
    if (p.level) parts.push(`Niveau estimé : ${p.level}`);
    if (Array.isArray(p.weakSpots) && p.weakSpots.length) parts.push(`Points à renforcer : ${p.weakSpots.join(', ')}`);
...
async function getEngagementText(uid: string): Promise<string> {
  try {
    const snap = await admin.firestore()
    ...
    const scored = snap.docs.map((d) => {
      const e = d.data() as {
        category?: string; title?: string; type?: string;
        scrollPctMax?: number; dwellSec?: number; mediaSec?: number;
      };
      const scroll = Math.min(e.scrollPctMax ?? 0, 100) / 100;
      const dwell = Math.min(e.dwellSec ?? 0, 600) / 600;
      const media = Math.min(e.mediaSec ?? 0, 1800) / 1800;
      return {
        category: e.category || 'général',
        title: e.title ?? 'contenu',
        type: e.type ?? 'contenu',
    ...
    scored.forEach((e) => {
      byCat.set(e.category, (byCat.get(e.category) ?? 0) + e.score);
    });
    ...
    const lines: string[] = [];
    if (topCats.length) lines.push(`- Catégories préférées : ${topCats.join(', ')}`);
...
async function persistAndSummarize(
  ...
  try {
    ...
    const prev: Array<{ role: string; content: string; ts: string }> = convSnap.data()?.messages ?? [];
    const msgCount: number = (convSnap.data()?.msgCountSinceSummary ?? 0) + 1;
    const messages = [
      ...prev,
    ...
    if (msgCount < SUMMARY_THRESHOLD) return;
    ...
    const prompt = [
      ...
      existing?.summary ? `Profil précédent: ${existing.summary}` : '',
    ...
    const raw = res.text ?? '';
    let parsed: { summary?: string; topics?: string[]; level?: string; weakSpots?: string[] };
    try {
    ...
    await profileRef.set({
      ...
      topics: Array.isArray(parsed.topics) ? parsed.topics.slice(0, 10) : (existing?.topics ?? []),
      level: parsed.level ?? existing?.level ?? '',
      weakSpots: Array.isArray(parsed.weakSpots) ? parsed.weakSpots.slice(0, 10) : (existing?.weakSpots ?? []),
      updatedAt: now,
    }, { merge: true });
    await convRef.set({ msgCountSinceSummary: 0 }, { merge: true });
  } catch (err) {
 ...
 */
async function reserveRequest(uid: string): Promise<QuotaSnapshot> {
  ...
  return await admin.firestore().runTransaction<QuotaSnapshot>(async (txn) => {
    const snap = await txn.get(rateLimitRef);
    const data = snap.data() ?? {};
    const sameDay = data.dayKey === dayKey;
    const dayCount: number = sameDay ? (data.dayCount ?? 0) : 0;
    const packBalance: number = data.packBalance ?? 0;
...
export const rysmo = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const reply = sanitizeInternalLinks(result.text ?? '', validPaths);
      ...
      if (errMsg.includes('429') || errMsg.includes('RESOURCE_EXHAUSTED')) {
        throw new HttpsError('resource-exhausted', 'Trop de requêtes côté serveur IA. Réessaie dans un moment.');
      }
      throw new HttpsError('internal', 'Le service IA est temporairement indisponible.');
    }
  }
 ...
 */
export const getRysmoQuota = onCall(
  { region: 'us-central1' },
  async (request) => {
    if (!request.auth) {
 ...
 */
export const clearRysmoMemory = onCall(
  { region: 'us-central1' },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentification requise.');
