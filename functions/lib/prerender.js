...
async function resolveBySlug(col, slug, lang) {
    const db = admin.firestore();
    if (lang === 'en') {
        const byEn = await db.collection(col).where('slug_en', '==', slug).limit(1).get();
        if (!byEn.empty && byEn.docs[0].data().status === 'published')
            return byEn.docs[0].data();
    }
...
function altMeta(seg, data, lang) {
    const frUrl = `${SITE_URL}/${seg}/${data.slug}`;
    const enUrl = `${SITE_URL}${(0, segments_1.enPath)(`/${seg}/${data.slug_en || data.slug}`)}`;
    return {
        canonical: data.canonicalUrl || (lang === 'en' ? enUrl : frUrl),
        altFr: frUrl,
        altEn: enUrl,
        lang,
    };
}
async function getContentMeta(path, lang) {
    var _a;
    // Blog post: /blog/:slug
    ...
    if (blogMatch) {
        ...
        if (!post)
            return null;
        return Object.assign({ title: `${post.metaTitle || post.title} | ${SITE_NAME}`, description: post.metaDescription || post.excerpt || '', ogType: 'article', ogImage: post.ogImage || post.coverImage || DEFAULT_OG_IMAGE, noIndex: post.noIndex, publishedAt: post.publishedAt, modifiedAt: post.updatedAt, h1: post.title, bodyText: post.excerpt + (post.content ? '\n\n' + stripMarkdown(post.content).slice(0, 2000) : ''), jsonLd: Object.assign({ '@context': 'https://schema.org', '@type': 'Article', headline: post.title, description: post.excerpt, image: post.coverImage, datePublished: post.publishedAt, dateModified: post.updatedAt || post.publishedAt, author: { '@type': 'Person', name: post.author || 'Max-Morrys' }, publisher: {
                    '@type': 'Organization',
                    name: SITE_NAME,
    ...
    if (formMatch) {
        ...
        return Object.assign({ title: `${f.metaTitle || f.title} | ${SITE_NAME}`, description: f.metaDescription || f.description || '', ogType: 'website', ogImage: f.ogImage || f.coverImage || DEFAULT_OG_IMAGE, noIndex: f.noIndex, publishedAt: f.publishedAt, modifiedAt: f.updatedAt, h1: f.title, bodyText: f.description + (longDesc ? '\n\n' + longDesc : ''), jsonLd: Object.assign({ '@context': 'https://schema.org', '@type': 'Course', name: f.title, description: f.description, provider: { '@type': 'Organization', name: SITE_NAME, url: SITE_URL }, educationalLevel: f.level, inLanguage: 'fr', image: f.coverImage, offers: {
                    '@type': 'Offer',
    ...
    if (podMatch) {
        ...
        if (!p)
            return null;
        return Object.assign({ title: `${p.metaTitle || p.title} | ${SITE_NAME}`, description: p.metaDescription || p.description || '', ogType: 'music.song', ogImage: p.ogImage || p.coverImage || DEFAULT_OG_IMAGE, noIndex: p.noIndex, publishedAt: p.publishedAt, modifiedAt: p.updatedAt, h1: p.title, bodyText: (p.description || '') + (p.transcript ? '\n\n' + stripMarkdown(p.transcript).slice(0, 2000) : ''), jsonLd: {
                '@context': 'https://schema.org',
                '@type': 'PodcastEpisode',
                name: p.title,
    ...
    if (vidMatch) {
        ...
        if (!v)
            return null;
        return Object.assign({ title: `${v.metaTitle || v.title} | ${SITE_NAME}`, description: v.metaDescription || v.description || '', ogType: 'video.other', ogImage: v.ogImage || v.thumbnailUrl || DEFAULT_OG_IMAGE, noIndex: v.noIndex, publishedAt: v.publishedAt, modifiedAt: v.updatedAt, h1: v.title, bodyText: v.description || '', jsonLd: {
                '@context': 'https://schema.org',
                '@type': 'VideoObject',
                name: v.title,
...
async function translateMetaToEn(meta) {
    const apiKey = googleAiKey.value();
    const sources = [meta.title, meta.description, meta.h1 || '', meta.bodyText || ''];
    if (!apiKey)
        return meta;
    try {
        const map = await (0, translate_1.translateCached)(sources, apiKey);
        const tr = (s) => (s ? map[s] || s : s);
        return Object.assign(Object.assign({}, meta), { title: tr(meta.title), description: tr(meta.description), h1: tr(meta.h1), bodyText: tr(meta.bodyText) });
    }
    catch (e) {
...
exports.prerender = (0, https_1.onRequest)({ region: 'europe-west1', memory: '256MiB', cpu: 1, secrets: [googleAiKey] }, async (req, res) => {
    try {
        const rawPath = req.path.replace(/\/+$/, '') || '/';
        const lang = rawPath === '/en' || rawPath.startsWith('/en/') ? 'en' : 'fr';
        // Chemin FR canonique (retire /en + remappe les segments EN).
        const canonicalPath = lang === 'en' ? (0, segments_1.canonicalizeSegments)(rawPath === '/en' ? '/' : rawPath.slice(3)) || '/' : rawPath;
        let meta = staticPages[canonicalPath] ? Object.assign({}, staticPages[canonicalPath]) : null;
        if (meta) {
            // Page statique : alternates + canonical selon la langue.
            const frUrl = `${SITE_URL}${canonicalPath}`;
            const enUrl = `${SITE_URL}${(0, segments_1.enPath)(canonicalPath)}`;
            meta.altFr = frUrl;
            meta.altEn = enUrl;
            meta.lang = lang;
        ...
        if (!meta) {
            ...
            meta = {
                ...
                lang,
                altFr: frUrl,
                altEn: `${SITE_URL}${(0, segments_1.enPath)(canonicalPath)}`,
            };
        }
