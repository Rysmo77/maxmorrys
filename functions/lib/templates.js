...
function titleBlock(p, size, color, accent, font = FONT_TITLE) {
 ...
 */
function shell(w, h, pad, grad, accent, contentInner, bgUri, curve = true) {
    let bgLayers;
    if (bgUri) {
        bgLayers =
 ...
 */
function flatShell(w, h, pad, acc, inner, deco = '') {
    return (`<div style="display:flex;position:relative;width:${w}px;height:${h}px;background:${C.bleuProfond};">` +
        deco +
...
function bodyBlock(text, size = 40, opacity = 0.86, marginTop = 34) {
    if (!text)
        return '';
 ...
 */
function posterTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    const size = Math.round(titleSize(p.title, h) * 1.08);
    const inner = `<div style="display:flex;flex-direction:column;">` + pill((p.eyebrow || 'MAX-MORRYS').toUpperCase(), acc, C.bleuProfond) + `</div>` +
        `<div style="display:flex;flex-direction:column;">` +
 ...
 */
function panelTpl(p, w, h, pad, bg, logo) {
    const acc = accentColor(p);
    ...
    const body = p.body
        ...
        : '';
    const panel = `<div style="display:flex;flex-direction:column;justify-content:flex-end;position:absolute;top:${photoH}px;left:0;width:${w}px;height:${h - photoH}px;background:${C.bleuProfond};padding:${pad}px;">` +
...
function slideProgress(index, total, acc, w) {
 ...
 */
function slideTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    const role = p.slideRole || 'body';
...
function statTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    ...
    const statSize = Math.max(72, Math.round(Math.min(h >= 1900 ? 300 : h >= 1300 ? 250 : 220, (w - 2 * pad) / (Math.max(value.length, 1) * 0.62))));
    const label = p.statLabel
        ...
        : '';
    const inner = `<div style="display:flex;flex-direction:column;">` + eyebrowBlock(p.eyebrow || 'PREUVE', acc) + `</div>` +
        `<div style="display:flex;flex-direction:column;">` +
...
function checklistTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    ...
    const rows = items
        .map((it, i) => `<div style="display:flex;align-items:center;margin-top:${i === 0 ? 0 : Math.round(dot * 0.42)}px;">` +
...
function versusTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    ...
    const col = (heading, items, color, bg) => `<div style="display:flex;flex-direction:column;flex:1;background:${bg};border-radius:28px;padding:${Math.round(pad * 0.62)}px;">` +
        ...
        items
            .slice(0, 5)
            .map((it, i) => `<div style="display:flex;color:${hexToRgba(C.blanc, 0.9)};font-size:32px;font-weight:500;line-height:1.3;margin-top:${i === 0 ? 0 : 20}px;font-family:${FONT_BODY};">${esc(it)}</div>`)
            .join('') +
        `</div>`;
    const inner = `<div style="display:flex;flex-direction:column;">` +
 ...
 */
function askTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    ...
    const opts = options.length
        ? `<div style="display:flex;flex-direction:column;margin-top:56px;">` +
            options
                .map((o, i) => `<div style="display:flex;align-items:center;justify-content:center;background:${i === 0 ? acc : hexToRgba(C.blanc, 0.12)};color:${i === 0 ? C.bleuProfond : C.blanc};font-size:44px;font-weight:800;padding:30px 40px;border-radius:999px;margin-top:${i === 0 ? 0 : 24}px;font-family:${FONT_BODY};">${esc(o)}</div>`)
                .join('') +
            `</div>`
        : '';
    const inner = `<div style="display:flex;flex-direction:column;">` + pill((p.eyebrow || 'MAX-MORRYS').toUpperCase(), acc, C.bleuProfond) + `</div>` +
...
function testimonialTpl(p, w, h, pad, _bg, logo) {
    const acc = accentColor(p);
    const author = p.authorName
        ? `<div style="display:flex;flex-direction:column;margin-top:44px;">` +
            ...
            `</div>`
        : '';
    const inner = `<div style="display:flex;flex-direction:column;">` + eyebrowBlock(p.eyebrow || 'TÉMOIGNAGE', acc) + `</div>` +
        `<div style="display:flex;flex-direction:column;">` +
        `<div style="display:flex;color:${acc};font-size:150px;font-weight:900;line-height:0.6;height:96px;font-family:${FONT_TITLE};">“</div>` +
