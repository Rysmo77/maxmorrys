  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => openSessionForm()} icon={<Plus className="w-4 h-4" />}>{t('sessions.new')}</Button>
      </div>

      {showSessionForm && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-neutral-900 dark:text-white">{editSession ? t('sessions.editTitle') : t('sessions.newTitle')}</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2 space-y-1">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.titleLabel')}</label>
              <input value={sessionForm.title} onChange={(e) => setSessionForm((p) => ({ ...p, title: e.target.value }))} placeholder={t('sessions.titlePlaceholder')} className={inputCls} />
            </div>
            <div className="sm:col-span-2 space-y-1">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.descriptionLabel')}</label>
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.datetimeLabel')}</label>
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.durationLabel')}</label>
              <input value={sessionForm.duration} onChange={(e) => setSessionForm((p) => ({ ...p, duration: e.target.value }))} placeholder={t('sessions.durationPlaceholder')} className={inputCls} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.statusLabel')}</label>
              <select value={sessionForm.status} onChange={(e) => setSessionForm((p) => ({ ...p, status: e.target.value as 'upcoming' | 'past' }))} className={inputCls}>
                <option value="upcoming">{t('sessions.statusUpcoming')}</option>
                <option value="past">{t('sessions.statusPast')}</option>
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.linkLabel')}</label>
            ...
            <div className="sm:col-span-2 space-y-2">
              <label className="text-xs font-semibold text-neutral-500">{t('sessions.imageLabel')}</label>
              ...
              {sessionImagePreview ? (
                ...
                <button type="button" onClick={() => sessionImageInputRef.current?.click()} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed border-neutral-300 dark:border-neutral-600 text-sm text-neutral-500 hover:border-brand-400 hover:text-brand-600 transition-colors">
                  <ImageIcon className="w-4 h-4" /> {t('sessions.importImage')}
                </button>
              )}
            </div>
          </div>
      ...
      {sessions.length === 0 && !showSessionForm ? (
        <Card><p className="text-center text-neutral-400 py-8">{t('sessions.empty')}</p></Card>
      ) : (
        <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Card key={session.id} hover padding="none">
              ...
              <div className="p-4">
                ...
                <div className="text-xs text-neutral-400 space-y-0.5 mb-3">
                  ...
                  {session.link && <a href={session.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-brand-500 hover:underline"><ExternalLink className="w-3 h-3" /> {t('sessions.connectionLink')}</a>}
                </div>
                <button
                ...
                {openSessionRegs === session.id && (
                  <div className="mt-2 space-y-1">
                    {(sessionRegs[session.id] ?? []).length === 0 ? (
                      <p className="text-xs text-neutral-400">{t('sessions.noRegistrations')}</p>
                    ) : (
