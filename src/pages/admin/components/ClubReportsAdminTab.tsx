...
export default function ClubReportsAdminTab() {
  ...
  const resolve = async (r: DmReport) => {
    ...
    try { await updateDmReportStatus(r.id, 'resolved'); addToast('success', 'Signalement résolu.'); }
    catch { addToast('error', 'Erreur.'); }
  };

  const removeReport = (r: DmReport) => {
    confirm.requestConfirm('Supprimer ce signalement ?', async () => {
      try { await deleteDmReport(r.id); setReports((prev) => prev.filter((x) => x.id !== r.id)); addToast('success', 'Signalement supprimé.'); }
  ...
  const removeMessage = (r: DmReport) => {
    confirm.requestConfirm('Supprimer le message signalé de la conversation ?', async () => {
      try {
        await deleteDmMessage(r.convId, r.messageId);
        await updateDmReportStatus(r.id, 'resolved').catch(() => null);
        setReports((prev) => prev.map((x) => x.id === r.id ? { ...x, status: 'resolved' } : x));
        addToast('success', 'Message supprimé et signalement résolu.');
      } catch (error: unknown) {
  ...
  if (reports.length === 0) return <Card><p className="text-center text-neutral-400 py-8">Aucun signalement.</p></Card>;
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                ...
                <Badge variant={r.status === 'open' ? 'error' : 'default'} size="sm">{r.status === 'open' ? 'À traiter' : 'Résolu'}</Badge>
              ...
              <p className="text-xs text-neutral-400">Signalé par <span className="font-mono">{r.reporterId.slice(0, 8)}…</span> · auteur <span className="font-mono">{r.reportedUserId.slice(0, 8)}…</span></p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-neutral-100 dark:border-neutral-800">
            <Button size="sm" variant="outline" onClick={() => removeMessage(r)} icon={<Trash2 className="w-3.5 h-3.5" />}>Supprimer le message</Button>
            {r.status === 'open' && <Button size="sm" variant="ghost" onClick={() => resolve(r)} icon={<Check className="w-3.5 h-3.5" />}>Marquer résolu</Button>}
            <Button size="sm" variant="ghost" onClick={() => removeReport(r)} icon={<Trash2 className="w-3.5 h-3.5" />}>Supprimer le signalement</Button>
          </div>
        </Card>
      ))}
      <ConfirmDialog open={confirm.open} onClose={confirm.closeConfirm} onConfirm={confirm.onConfirm} title="Confirmer" message={confirm.message} confirmLabel="Confirmer" />
    </div>
  );
}
