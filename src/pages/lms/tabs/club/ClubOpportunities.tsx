    ...
    <motion.div className="space-y-4" variants={staggerContainer} initial="hidden" animate="visible">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-neutral-900 dark:text-white">{t('opportunities.title')}</h3>
        </div>
        <button onClick={() => setShowForm((v) => !v)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-plum-600 hover:bg-plum-700 text-white text-xs font-semibold transition-colors">
          {showForm ? <X className="w-4 h-4" weight="bold" /> : <Plus className="w-4 h-4" weight="bold" />} {showForm ? t('opportunities.close') : t('opportunities.publish')}
        </button>
      </div>

      {showForm && (
        <div className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-2xl p-4 space-y-3">
          <input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder={t('opportunities.titlePlaceholder')} className={inputCls} />
          <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={3} placeholder={t('opportunities.descriptionPlaceholder')} className={cn(inputCls, 'resize-y')} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <select value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as ClubOpportunity['type'] }))} className={inputCls}>
              <option value="mission">{t('opportunities.typeMission')}</option>
              <option value="emploi">{t('opportunities.typeEmploi')}</option>
              <option value="partenariat">{t('opportunities.typePartenariat')}</option>
              <option value="autre">{t('opportunities.typeAutre')}</option>
            </select>
            <input value={form.budget} onChange={(e) => setForm((p) => ({ ...p, budget: e.target.value }))} placeholder={t('opportunities.budgetPlaceholder')} className={inputCls} />
          </div>
          <input value={form.contact} onChange={(e) => setForm((p) => ({ ...p, contact: e.target.value }))} placeholder={t('opportunities.contactPlaceholder')} className={inputCls} />
          <div className="flex justify-end">
            <button onClick={handleSubmit} disabled={submitting || !form.title.trim() || !form.description.trim() || !form.contact.trim()} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-plum-600 hover:bg-plum-700 text-white text-sm font-semibold disabled:opacity-50">
              {submitting ? <CircleNotch className="w-4 h-4 animate-spin" /> : <PaperPlaneTilt className="w-4 h-4" weight="fill" />} {t('opportunities.publish')}
            </button>
          </div>
        </div>
      )}
      ...
      {items.length === 0 ? (
        <ClubEmptyState icon={Briefcase} title={t('opportunities.emptyTitle')} subtitle={t('opportunities.emptySubtitle')} action={(
          <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-plum-600 hover:bg-plum-700 text-white text-sm font-semibold transition-colors">
            <Plus className="w-4 h-4" weight="bold" /> {t('opportunities.publishOpportunity')}
          </button>
        )} />
      ) : (
            ...
            <motion.div key={item.id} variants={staggerItem} className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-2xl p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-plum-100 dark:bg-plum-900/30 text-plum-700 dark:text-plum-300">{t(TYPE_LABEL_KEYS[item.type])}</span>
