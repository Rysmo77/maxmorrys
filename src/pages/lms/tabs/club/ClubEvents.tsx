...
export default function ClubEvents({ data }: ClubEventsProps) {
  ...
  if (clubEvents.length === 0) {
    return <ClubEmptyState icon={CalendarBlank} title="Aucun événement prévu" subtitle="Les prochains événements du Club apparaîtront ici." />;
  }
          ...
          <motion.div key={event.id} variants={staggerItem} className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-2xl overflow-hidden relative hover:-translate-y-0.5 hover:shadow-md transition-all duration-300">
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {event.link && <a href={event.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold text-plum-600 dark:text-plum-400 hover:underline"><ArrowSquareOut className="w-3.5 h-3.5" weight="bold" /> Voir l'événement</a>}
