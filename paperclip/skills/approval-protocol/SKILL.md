...
## Mécanisme maxmorrys — piloté par l'état en base (PAS d'appel direct à une API sortante)
Le garde-fou est **piloté par le `Status`** des enregistrements NocoDB, relayé par les workflows n8n. Tu ne « publies » jamais : tu **poses un brouillon complet** et tu passes le `Status` au point de hand-off. Un workflow notifie le board, et **seule l'approbation** déclenche l'action.
...
2. **Passe le `Status` à `prêt_à_valider`.** C'est le seul geste que tu fais.
...
4. **Tu t'arrêtes là.** Tu ne passes JAMAIS un `Status` à `validé`, tu n'appelles jamais WF-SOCIAL-05 / WF-EMAIL-SEND / WF-WHATSAPP-SEND toi-même.
...
## Interdits
- Ne jamais passer un `Status` directement à `validé` / `publié`.
